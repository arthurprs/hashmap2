...
impl<K, V> RawBucket<K, V> {
    unsafe fn offset(self, count: isize) -> RawBucket<K, V> {
        RawBucket {
            triple: self.triple.offset(count),
...
impl<K, V, M: Deref<Target = RawTable<K, V>>> Bucket<K, V, M> {
    ...
    pub fn peek(self) -> BucketState<K, V, M> {
        match unsafe { (*self.raw.triple).0 } {
            EMPTY_BUCKET => {
                Empty(EmptyBucket {
                    raw: self.raw,
                    idx: self.idx,
    ...
    where M: Put<K, V>
{
    ...
    pub fn put(mut self, hash: SafeHash, key: K, value: V) -> FullBucket<K, V, M> {
        unsafe {
            ptr::write(self.raw.triple, (hash.inspect(), key, value));
...
impl<K, V, M: Deref<Target = RawTable<K, V>>> FullBucket<K, V, M> {
    ...
    pub fn hash(&self) -> SafeHash {
        unsafe { SafeHash { hash: (*self.raw.triple).0 } }
    }

    /// Gets references to the key and value at a given index.
    pub fn read(&self) -> (&K, &V) {
        unsafe { (&(*self.raw.triple).1, &(*self.raw.triple).2) }
    }
}
...
impl<'t, K, V> FullBucket<K, V, &'t mut RawTable<K, V>> {
    ...
    pub fn take(mut self) -> (EmptyBucket<K, V, &'t mut RawTable<K, V>>, K, V) {
        ...
        unsafe {
            (*self.raw.triple).0 = EMPTY_BUCKET;
            let (_, k, v) = ptr::read(self.raw.triple);
            (EmptyBucket {
                raw: self.raw,
                idx: self.idx,
                table: self.table,
    ...
    where M: Put<K, V>
{
    pub fn replace(&mut self, h: SafeHash, k: K, v: V) -> (SafeHash, K, V) {
        unsafe {
            let (old_hash, old_key, old_val) = ptr::replace(self.raw.triple, (h.hash, k, v));

            (SafeHash{hash: old_hash}, old_key, old_val)
        }
    }
}
    ...
    where M: Deref<Target = RawTable<K, V>> + DerefMut
{
    ...
    pub fn read_mut(&mut self) -> (&mut K, &mut V) {
        unsafe { (&mut (*self.raw.triple).1, &mut (*self.raw.triple).2) }
    }
}
    ...
    where M: Deref<Target = RawTable<K, V>>
{
    ...
    pub fn shift(mut self) -> Option<GapThenFull<K, V, M>> {
        unsafe {
            ptr::copy_nonoverlapping(self.full.raw.triple, self.gap.raw.triple, 1);
            (*self.full.raw.triple).0 = EMPTY_BUCKET;
        }
...
fn calculate_allocation(hash_size: usize,
    ...
    let hash_offset = 0;
    let (_, end_of_keys, oflo) = calculate_offsets(hash_size, keys_size, keys_align);

    let align = cmp::max(hash_align, keys_align);

    (align, hash_offset, end_of_keys, oflo)
}
...
impl<K, V> RawTable<K, V> {
    ...
    unsafe fn new_uninitialized(capacity: usize) -> RawTable<K, V> {
        if capacity == 0 {
            return RawTable {
                size: 0,
                capacity: 0,
                hashes: Unique::new(EMPTY as *mut _),
        ...
        let triples_size = capacity * size_of::<(usize, K, V)>();
        let hash_offset = 0;
        ...
        // let size_of_bucket = size_of::<usize>().checked_add(size_of::<(K, V)>()).unwrap();
        // assert!(size >=
        //         capacity.checked_mul(size_of_bucket)
        //             .expect("capacity overflow"),
        //         "capacity overflow");

        let buffer = allocate(triples_size, align_of::<(usize, K, V)>());
        if buffer.is_null() {
            ::alloc::oom()
        }
        ...
        RawTable {
            capacity: capacity,
            size: 0,
            hashes: Unique::new(hashes as *mut _),
            marker: marker::PhantomData,
        }
    }
    ...
    fn first_bucket_raw(&self) -> RawBucket<K, V> {
        ...
        // let keys_size = self.capacity * size_of::<(K, V)>();
        //
        let buffer = *self.hashes as *mut u8;
        // let (keys_offset, _, oflo) =
        //     calculate_offsets(hashes_size, keys_size, align_of::<(K, V)>());
        // debug_assert!(!oflo, "capacity overflow");
        // unsafe {
            RawBucket {
                triple: buffer as *mut _,
                _marker: marker::PhantomData,
            }
        // }
    }
...
impl<'a, K, V> Iterator for RawBuckets<'a, K, V> {
    ...
    fn next(&mut self) -> Option<RawBucket<K, V>> {
        while self.raw.triple != self.hashes_end {
            unsafe {
...
impl<'a, K, V> Iterator for RevMoveBuckets<'a, K, V> {
    ...
    fn next(&mut self) -> Option<(K, V)> {
        ...
        loop {
            debug_assert!(self.raw.triple != self.hashes_end);

            unsafe {
                self.raw = self.raw.offset(-1);

                if (*self.raw.triple).0 != EMPTY_BUCKET {
                    self.elems_left -= 1;
                    let (_, k, v) = ptr::read(self.raw.triple);
                    return Some((k, v));
                }
...
impl<'a, K, V> Iterator for Iter<'a, K, V> {
    ...
    fn next(&mut self) -> Option<(&'a K, &'a V)> {
        self.iter.next().map(|bucket| {
            self.elems_left -= 1;
            unsafe { (&(*bucket.triple).1, &(*bucket.triple).2) }
        })
    }
...
impl<'a, K, V> Iterator for IterMut<'a, K, V> {
    ...
    fn next(&mut self) -> Option<(&'a K, &'a mut V)> {
        self.iter.next().map(|bucket| {
            self.elems_left -= 1;
            unsafe { (&(*bucket.triple).1, &mut (*bucket.triple).2) }
        })
    }
...
impl<K, V> Iterator for IntoIter<K, V> {
    ...
    fn next(&mut self) -> Option<(SafeHash, K, V)> {
        self.iter.next().map(|bucket| {
            self.table.size -= 1;
            unsafe {
                let (h, k, v) = ptr::read(bucket.triple);
                (SafeHash { hash: h }, k, v)
            }
        })
    }
...
impl<'a, K, V> Iterator for Drain<'a, K, V> {
    ...
    fn next(&mut self) -> Option<(SafeHash, K, V)> {
        self.iter.next().map(|bucket| {
            unsafe {
                (**self.table).size -= 1;
                let (_, k, v) = ptr::read(bucket.triple);
                (SafeHash { hash: ptr::replace(&mut (*bucket.triple).0, EMPTY_BUCKET) }, k, v)
            }
        })
    }
                ...
                while buckets.index() != cap {
                    match buckets.peek() {
                        Full(full) => {
                            ...
                            ptr::write(new_buckets.raw.triple, (h.inspect(), k, v));
                        }
                        Empty(..) => {
                            (*new_buckets.raw.triple).0 = EMPTY_BUCKET;
                        }
                    }
                    new_buckets.next();
                    buckets.next();
